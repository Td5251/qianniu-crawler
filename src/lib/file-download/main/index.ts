...
class FileDownload {
    ...
    if (await this.checkWhetherHasDownloaded(options)) {
      ...
      if (options.feedbackProgressToRenderer && browserWindow) {
        browserWindow.webContents.send(
          "file-download-progress-feedback",
          options.uuid,
          result.fileSize,
          result.fileSize
        );
  ...
  protected checkParam(options: fdTypes.Options): boolean {
    if (!options.uuid) return false;

    if (!options.url) return false;

    if (!options.savePath) return false;

    if (options.verifyMd5 && !options.md5) return false;

    return true;
  }

  protected async checkWhetherHasDownloaded(
    options: fdTypes.Options
  ): Promise<boolean> {
  ...
  protected async streamWithProgress(
    ...
    for (;;) {
      ...
      if (result.done) {
        if (options.feedbackProgressToRenderer && browserWindow) {
          browserWindow.webContents.send(
            "electron-file-download-download-file-progress-feedback",
            options.uuid,
            bytesDone,
            finalLength
          );
...
ipcMain.handle(
  "electron-file-download-async-download-file",
  async (event, options: fdTypes.Options) => {
